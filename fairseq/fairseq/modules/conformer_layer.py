...
class VAConformerEncoderLayer(torch.nn.Module):
    """Conformer block based on https://arxiv.org/abs/2005.08100. We currently don't support relative positional encoding in MHA"""
    ...
    def __init__(
        ...
        """
        self.pos_enc_type = pos_enc_type
        super(VAConformerEncoderLayer, self).__init__()
        ...
        if attn_type == "espnet":
            if self.pos_enc_type == "rel_pos":
                ...
                self.cross_attn_audio_video = RelPositionMultiHeadedAttention(embed_dim, attention_heads,
                                                                              dropout=dropout, )
                self.cross_attn_video_audio = RelPositionMultiHeadedAttention(embed_dim, attention_heads,
                                                                              dropout=dropout, )

            elif self.pos_enc_type == "rope":
                self.self_attn_audio = RotaryPositionMultiHeadedAttention(embed_dim, attention_heads, dropout=dropout,
                                                                          precision=use_fp16)
                self.self_attn_video = RotaryPositionMultiHeadedAttention(embed_dim, attention_heads, dropout=dropout,
                                                                          precision=use_fp16)
                self.cross_attn_audio_video = RotaryPositionMultiHeadedAttention(embed_dim, attention_heads,
                                                                                 dropout=dropout, precision=use_fp16)
                self.cross_attn_video_audio = RotaryPositionMultiHeadedAttention(embed_dim, attention_heads,
                                                                                 dropout=dropout, precision=use_fp16)
            ...
            # Default to fairseq MHA
            self.self_attn_audio = MultiheadAttention(embed_dim, attention_heads, dropout=dropout,
                                                      self_attention=True, )
            self.self_attn_video = MultiheadAttention(embed_dim, attention_heads, dropout=dropout,
                                                      self_attention=True, )
            self.cross_attn_audio_video = MultiheadAttention(embed_dim, attention_heads, dropout=dropout,
                                                             encoder_decoder_attention=True, )
            self.cross_attn_video_audio = MultiheadAttention(embed_dim, attention_heads, dropout=dropout,
                                                             encoder_decoder_attention=True, )
...
class VAConformerWav2Vec2EncoderLayer(VAConformerEncoderLayer):
    """Encoder layer for Wav2vec2 encoder"""
...
class CoAttention(nn.Module):
    ...
    def forward(
        ...
        residual_audio = self.conv_audio(torch.matmul(attn_map, v).transpose(1, 2).contiguous()).transpose(1,
                                                                                                           2).contiguous()

        # video residual
        q, k, v = x, x_v.transpose(1, 2).contiguous(), x_v
        attn_map = torch.softmax(torch.matmul(q, k) / math.sqrt(self.embedding_dim), dim=-1)
        residual_video = self.conv_video(torch.matmul(attn_map, v).transpose(1, 2).contiguous()).transpose(1,
                                                                                                           2).contiguous()
