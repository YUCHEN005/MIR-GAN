...
class ModalDiscriminator(nn.Module):
    def __init__(self, dim=768):
        ...
        self.proj2 = nn.Linear(dim // 4, dim // 16)
        self.output_layer = nn.Linear(dim // 16, 1)
        self.sigmoid = nn.Sigmoid()

    def forward(self, x):
        x = self.output_layer(self.proj2(self.proj1(x)))
        out = self.sigmoid(x)
        return out
...
class AVHubertModel(BaseFairseqModel):
    def __init__(
        ...
        **kwargs
        ...
        self.va_encoder = VATransformerEncoder(cfg)

        self.mir_generator = MIRGenerator(num_layers=cfg.mir_layers, embedding_dim=cfg.encoder_embed_dim)
        ...
        # discriminator
        self.modal_discriminator = ModalDiscriminator(cfg.encoder_embed_dim)
    ...
    def extract_finetune(self, source, padding_mask=None, mask=False, ret_conv=False, output_layer=None):
        ...
        x_a, x_v = features_audio.transpose(1, 2), features_video.transpose(1, 2)
        ...
        ### VA encoder
        x_a, x_v, _ = self.va_encoder(x_a, x_v)

        ### MIR generator
        x = self.mir_generator(x_a, x_v, x)
        ...
        ### ASR encoder
        x_enc, _ = self.encoder(
            x + x_a + x_v,
            padding_mask=padding_mask,
            layer=None if output_layer is None else output_layer - 1
        )

        return x_enc, padding_mask, (x_a, x_v, x)
